...
const JWT_SECRET =
  "4cba39b0806e0def0a7248ec75550485e93b5fb00dcd83f132693c1e8f9c3b3f";
  ...
  try {
    jwt.verify(token, JWT_SECRET, (err, decoded) => {
      ...
      const userMessages = messages.filter(
        (message) =>
          userId === message.receiverID || userId === message.senderID,
      );
...
export const updateMessage = async (req, res) => {
  ...
  if (!message || message.length < 1) {
    return res
      .status(400)
      .json({ error: "Please provide an appropriate message" });
  }
  const autHeader = req.headers.authorization;
  if (!autHeader || !autHeader.startsWith("Bearer ")) {
    res.json(400).json({ error: "Login Error!" });
  ...
  try {
    jwt.verify(token, JWT_SECRET, (err, decoded) => {
      ...
      if (!messageToUpdate) {
        return res
          .status(404)
          .json({ error: "There is no message with this id." });
      }
